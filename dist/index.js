        ...
        tools: [
            {
                ...
                description: "Validate the syntax of a Mermaid diagram code string using the same " +
            ...
            },
            {
                ...
                description: "Start a new Mermaid diagram fix session. Submits the initial code and performs first validation. " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        maxAttempts: {
                            type: "number",
                            description: "Maximum number of fix attempts allowed (default: 10, max: 50).",
                        },
                    },
                    required: ["code"],
                },
            },
            {
                ...
                description: "Submit line-level fixes to an ongoing fix session. Only modifies specified lines, " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        lineFixes: {
                            ...
                            description: "Array of line fixes: specify lineNumber (1-based) and new content for that line.",
                        },
                    },
            ...
            },
            {
                ...
                description: "Get the final valid code from a completed session and clean up resources.",
                inputSchema: {
                    type: "object",
                    properties: {
                        requestId: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === FIX_START_TOOL_NAME) {
            ...
            const result = await fixEngine.submitInitialCode(parsed.data.code, parsed.data.maxAttempts);
            return {
                content: [
                    {
                        type: "text",
        ...
        if (name === FIX_SUBMIT_TOOL_NAME) {
            ...
            const result = await fixEngine.submitFixes(parsed.data.requestId, parsed.data.lineFixes);
            return {
                content: [
                    {
                        type: "text",
